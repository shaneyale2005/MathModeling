# advanced_model_visualization.py
...
SOFT_COLORS = {
    ...
    'sentiment': ['#ffb3ba', '#bae1ff', '#b8e6b8'],  # 负面、中性、正面
...
def plot_sentiment_model_analysis(df):
    ...
    with plt.rc_context(rc={'figure.dpi': 250, 
        ...
        # 11. 互动量分布的小提琴图
        ax = axes[10]
        df_sample = df.sample(min(1000, len(df)))  # 采样以提高性能
...
def plot_model_comparison_analysis(warning_model, df):
    ...
    with plt.rc_context(rc={'figure.dpi': 250, 
        ...
        # 2. 准确性对比
        ax = axes[1]
        accuracy_scores = [0.85, 0.78]  # 模拟准确性分数
        ...
        # 4. 资源消耗对比
        ax = axes[3]
        resource_scores = [3, 7]  # 资源消耗评分（越高消耗越大）
        ...
        sentiment_scores = [0.85, 0.9, 0.8, 0.9, 0.7]  # 情感模型评分
        warning_scores = [0.78, 0.7, 0.85, 0.6, 0.8]   # 预警模型评分
        ...
        sentiment_suitability = [8, 9, 6, 5]  # 情感模型适用性
        warning_suitability = [7, 6, 9, 9]   # 预警模型适用性
